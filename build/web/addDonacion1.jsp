...
<%@page import="modelo.TipoNecesidad"%>
<%@page import="DAO.NecesidadesDAO"%>
<%@page import="modelo.Institucion"%>
<%@page import="DAO.InstitucionesDAO"%>
<%@page import="java.util.Iterator"%>
<%@page import="modelo.Estados"%>
<%@page import="java.util.List"%>
<%@page import="java.util.List"%>
<%@page import="DAO.EstadosDAO"%>
        ...
        <div class="mb-3">
            ...
            <div>
                <form action="ControladorDonaciones" method ="POST">
                    ...
                    <!-- ID -->
                    <input type="hidden" name="txtid" value="1" required>
                    Estado:
                    <select name = "txtest" class="col-md-12" required>

                        <option value=""></option>
                        <%
                            EstadosDAO dao = new EstadosDAO();
                            List<Estados> list1 = dao.listar();
                            Iterator<Estados> iter1 = list1.iterator();
                            Estados emp = null;
                            while (iter1.hasNext()) {
                                emp = iter1.next();
                        %>
                    ...
                    Institución:
                    <select name = "txtins" class="col-md-12" required>

                        <option value=""></option>
                        <%
                            InstitucionesDAO dao3 = new InstitucionesDAO();
                            List<Institucion> list13 = dao3.listar();
                            Iterator<Institucion> iter13 = list13.iterator();
                            Institucion emp3 = null;
                            while (iter13.hasNext()) {
                                emp3 = iter13.next();
                        %>
                        <option  value="<%=emp3.getPlantel()%>"><%=emp3.getNombreInst()%></option>
                        <%}%>
                    </select>
                    
                    Necesidad:
                    <select name = "txtnec" class="col-md-12" required>

                        <option value=""></option>
                        <%
                            NecesidadesDAO dao1 = new NecesidadesDAO();
                            List<TipoNecesidad> list11 = dao1.listar();
                            Iterator<TipoNecesidad> iter11 = list11.iterator();
                            TipoNecesidad emp1 = null;
                            while (iter11.hasNext()) {
                                emp1 = iter11.next();
                        %>
                        <option  value="<%=emp1.getId()%>"><%=emp1.getNombre()%></option>
                        <%}%>
                    </select>
                    Descripción:
                    <input type="text" name="txtdes" required style="HEIGHT: 98px">
                    Nombre contacto:
                    <input type="text" name="txtcon" required>
                    Número contacto:
                    <input type="text" name="txttel" required>

                    <input class="botonCrud" type="submit" name="accion" value="Guardar1">
                </form>
                <div class="col-md-12">
                    <a class="read_more ye_b5n " href="donaciones1.jsp" style="width: 100%"> Cancelar</a>
                </div>
            </div>
        </div>        
    </div>
